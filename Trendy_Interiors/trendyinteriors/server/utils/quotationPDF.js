...
function checkSpace(doc, heightNeeded, onNewPage) {
...
function drawDataTable(doc, headers, rows, colWidths, totalRow = null) {
  ...
  const requiredSpace = headerHeight + (rows.length * rowHeight) + (totalRow ? rowHeight + 10 : 0);
  ...
  // Header
  doc.rect(MARGIN, currentY, CONTENT_W, headerHeight).fill(NAVY);
  let currentX = MARGIN;
  headers.forEach((h, i) => {
    doc.fillColor(WHITE).font("Helvetica-Bold").fontSize(9);
    const align = i === 0 ? "left" : (i === headers.length - 1 ? "right" : "center");
    doc.text(h, currentX + 8, currentY + 7, { width: colWidths[i] - 16, align });
  ...
  doc.lineWidth(0.4).strokeColor(BORDER);
  rows.forEach((row, ri) => {
    if (checkSpace(doc, rowHeight)) {
      currentY = doc.y; // Update Y if page breaks
    }
    ...
    row.forEach((cell, ci) => {
      ...
      const isFirst = ci === 0;
      doc.fillColor(DARK_TEXT).font(isFirst ? "Helvetica" : "Helvetica").fontSize(9);
      const align = isFirst ? "left" : (ci === row.length - 1 ? "right" : "center");
      
      // We manually bold the first column in Python script, here we just use normal unless specific.
  ...
  // Total Row
  if (totalRow) {
    if (checkSpace(doc, rowHeight + 10)) {
      currentY = doc.y;
    }
    doc.rect(MARGIN, currentY, CONTENT_W, rowHeight + 6).fill(GOLD_LITE);
    
    doc.lineWidth(1).strokeColor(GOLD);
    doc.moveTo(MARGIN, currentY).lineTo(MARGIN + CONTENT_W, currentY).stroke();
    doc.moveTo(MARGIN, currentY + rowHeight + 6).lineTo(MARGIN + CONTENT_W, currentY + rowHeight + 6).stroke();
...
const generateQuotationPDF = async (estimator, res, callback) => {
  try {
    ...
    doc.fillColor(WHITE).font("Helvetica-Bold").fontSize(9);
    doc.text("Description", MARGIN + 10, cy + 7, { width: CONTENT_W * 0.65 - 20, align: "left" });
    doc.text("Amount", MARGIN + CONTENT_W * 0.65 + 10, cy + 7, { width: CONTENT_W * 0.35 - 20, align: "right" });
    cy += 24;

    doc.lineWidth(0.4).strokeColor(BORDER);
    costSummaryTableRows.forEach((row, idx) => {
      const bg = idx % 2 === 0 ? ZEBRA : WHITE;
      doc.rect(MARGIN, cy, CONTENT_W, 22).fillAndStroke(bg, BORDER);
      doc.rect(MARGIN, cy, CONTENT_W * 0.65, 22).stroke();
      doc.fillColor(DARK_TEXT).font("Helvetica").fontSize(9);
      doc.text(row[0], MARGIN + 10, cy + 6, { width: CONTENT_W * 0.65 - 20, align: "left" });
      doc.text(row[1], MARGIN + CONTENT_W * 0.65 + 10, cy + 6, { width: CONTENT_W * 0.35 - 20, align: "right" });
      cy += 22;
    });
    ...
    doc.lineWidth(1).strokeColor(GOLD);
    doc.moveTo(MARGIN, cy).lineTo(MARGIN + CONTENT_W, cy).stroke();
    doc.moveTo(MARGIN, cy + 30).lineTo(MARGIN + CONTENT_W, cy + 30).stroke();
    
    doc.fillColor(NAVY).font("Helvetica-Bold").fontSize(11);
    doc.text("GRAND TOTAL", MARGIN + 10, cy + 10, { width: CONTENT_W * 0.65 - 20, align: "left" });
    doc.text(fmt(quotation.costs.grand_total), MARGIN + CONTENT_W * 0.65 + 10, cy + 10, { width: CONTENT_W * 0.35 - 20, align: "right" });
    doc.y = cy + 40;
