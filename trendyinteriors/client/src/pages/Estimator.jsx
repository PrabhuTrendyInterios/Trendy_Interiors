...
const Estimator = () => {
  ...
  const [selectedLayoutMaterials, setSelectedLayoutMaterials] = useState({}); // roomId -> { materialId: boolean }
      ...
      try {
        ...
        if (quote && Array.isArray(quote.lineItems)) {
          ...
          quote.lineItems.forEach((item) => {
            ...
            if (item.roomId && item.roomId !== 'global-addons' && item.layout) {
              ...
              if (layoutData.materials.length > 0) {
                initialLayoutMaterials[item.roomId] = buildDefaultLayoutMaterialSelection(layoutData.materials);
              }
            }
          });
  ...
  }, [selectedPackageComponents, selectedLayoutMaterials]);
