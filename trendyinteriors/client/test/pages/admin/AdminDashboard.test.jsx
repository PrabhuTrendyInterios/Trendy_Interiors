...
jest.mock('./components/AdminNavigation', () => ({ activeTab }) => <div>Nav {activeTab}</div>);
jest.mock('./components/FormCard', () => ({ title, children }) => <section><h2>{title}</h2>{children}</section>);
jest.mock('./components/DragDropUpload', () => () => <div>DragDropUpload Mock</div>);
jest.mock('./components/MultiImageUpload', () => () => <div>MultiImageUpload Mock</div>);
jest.mock('./components/Toast', () => ({ message }) => <div>{message}</div>);
jest.mock('./components/DeleteConfirmationModal', () => () => null);
